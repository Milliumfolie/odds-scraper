"""
Functions de gestion de la base de données des noms d'équipe/joueur/compétition
"""
import json
import sqlite3
import urllib
...
def get_id_from_competition_name(competition, sport):
    """
    Return the id and the name according to comparateur-de-cotes.fr. For
    exemple, "Ligue 1" becomes "France - Ligue 1"
    """
...
def get_competition_id(name, sport):
    """
    Return the id of a competition
    """
    conn = sqlite3.connect(sb.PATH_DB)
    c = conn.cursor()
    c.execute("""
...
def get_competition_url(name, sport, site):
    """
    Return the 'url' of a competition for a given sport.
    """
    conn = sqlite3.connect(sb.PATH_DB)
    c = conn.cursor()
    c.execute("""
...
def get_sport_by_url(url, site):
    conn = sqlite3.connect(sb.PATH_DB)
    c = conn.cursor()
    c.execute("""
    SELECT sport FROM competitions WHERE url_{}='{}'
    """.format(site, url))
    return c.fetchone()[0]
...
def is_id_in_db(_id):
    """
    Verify if id is in the given database
    """
    conn = sqlite3.connect(sb.PATH_DB)
    c = conn.cursor()
    c.execute("""
...
def get_all_competitions(sport):
    ...
    c = conn.cursor()
    c.execute("""
    SELECT competition FROM competitions WHERE sport='{0}' AND competition<>'Tout le {0}' AND url_betcity IS NOT NULL
    """.format(sport))
...
def get_competition_name_by_url(url, bookmaker):
    """
    Returns the name of a competition for a given URL on a specific bookmaker's site.
    """
    conn = sqlite3.connect(sb.PATH_DB)
    c = conn.cursor()

    # Correcting the SQL query to use the provided 'url' and 'bookmaker' parameters
    c.execute("SELECT competition FROM competitions WHERE url_{} = ?".format(bookmaker), (url,))

    try:
        return c.fetchone()[0]
    except TypeError:
        # If no result is found, return None or an appropriate default value
        return None


def get_sport_by_competition_name(competition_name):
    """
    Retourne l'url d'une competition donnée sur un site donné
    """
    conn = sqlite3.connect(sb.PATH_DB)
    c = conn.cursor()
    c.execute("""
    SELECT sport FROM competitions WHERE competition='{}'
...
def get_all_current_competitions(sport):
    ...
    for league in parsed["leagues"]:
        ...
        if sport in league["strSport"]:            
            id_league = int(league["idLeague"])
...
def get_teams(sport):   
    ...
    WHERE sport='{}'""".format(sport))
    competitions = c.fetchall()
    competitions = [x[0].replace(" ","%20") for x in competitions]
    ...
    for competition in competitions:
        ...
        url = "https://www.thesportsdb.com/api/v1/json/3/search_all_teams.php?l="+competition
        print(url)
...
def is_player_in_db(player):
    conn = sqlite3.connect(sb.PATH_DB)
    c = conn.cursor()
    c.execute("SELECT name FROM players WHERE name = ?", (player,))
    if c.fetchall():
        return True
    return False
...
def is_player_added_in_db(player, site):
    conn = sqlite3.connect(sb.PATH_DB)
    c = conn.cursor()
    c.execute("SELECT name FROM players WHERE name_{}=?".format(site), (player,))
...
def get_close_player_name(name, site):
    ...
    if "." in name:
        ...
        for line in c.fetchall():
            print(line[0])
            if re.match(unidecode(last_name).lower(), unidecode(line[0].split(",")[0]).lower()):
    ...
    #    return results
    if len(split_name) == 2: #and len(split_name[0]) == 1:
        init_first_name = split_name[1]
        last_name = split_name[0].strip()
        reg_exp = r'{}\s{}'.format(init_first_name, last_name)
        conn = sqlite3.connect(sb.PATH_DB)
        c = conn.cursor()
        c.execute("""
        SELECT name FROM players WHERE name_{} IS NULL
