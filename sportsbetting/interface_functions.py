...
from sportsbetting.database_functions import get_all_current_competitions, get_all_competitions, get_sport_by_competition_name
from sportsbetting.user_functions import (best_match_under_conditions_GUI, best_match_under_conditions2,
                                          best_match_freebet_GUI, best_stakes_match,
                                          ...
                                          best_combine_booste, trj_match, gain_middle, best_matches_freebet_one_site, get_values,
                                          best_matches_freebet2, best_match_defi_rembourse_ou_gagnant, best_combine_booste_progressif)

from sportsbetting.performances import get_surebets_players_basketball, get_surebets_players_football
...
def infos(result):
    ...
    for i, line in enumerate(lines):
        if "}}" in line:
            break
    print("Data passed to eval:", "".join(lines[1:i + 1]))
...
def best_match_under_conditions_interface(window, values):
    ...
    try:
        ...
        if one_site:
            ...
            best_match_under_conditions_GUI(site, minimum_odd, bet, sport, date_max, time_max, date_min,
                                        time_min, one_site)
...
def best_match_freebet_interface(window, values):
    ...
    try:
        ...
        if split_freebet:
            ...
            best_match_freebet_GUI(site, freebet, sport)
        else:
...
def filter_surebets_by_market(surebets, market):
    filtered_surebets = {}
    for key, value in surebets.items():
        if market in key:
            filtered_surebets[key] = value
        #if "Asian" in market:
    return filtered_surebets
...
def find_perf_players(window, values):
    ...
    print("test")
    sport = get_sport_by_competition_name(values["COMPETITION_PERF"])
    
    print(sport)
    
    if sport == "football":
        sb.SUREBETS, sb.MIDDLES = get_surebets_players_football(values["SITES_PERF"], values["COMPETITION_PERF"])
        
        
    if sport == "basketball":
        sb.SUREBETS, sb.MIDDLES = get_surebets_players_basketball(values["SITES_PERF"], values["COMPETITION_PERF"])
    ...
    #window["MAIN_PERF"].update(filter_surebets_by_market(sb.SUREBETS))
    window["PERIOD_PERF"].update(filter_surebets_by_market(sb.SUREBETS,"Half"))
    
    
def find_player_props(window, values):
    if not values["SITES_PLAYER_PROPS"]:
        return
    if not values["COMPETITION_PLAYER_PROPS"]:
        return
    #sport = get_sport_by_competition_name(values["COMPETITION_PLAYER_PROPS"])
    sport = "football"
    print(sport)
    if sport == "football":
        sb.SUREBETS, sb.MIDDLES = get_surebets_players_football(values["SITES_PLAYER_PROPS"], values["COMPETITION_PLAYER_PROPS"])
    if sport == "basketball":
        sb.SUREBETS, sb.MIDDLES = get_surebets_players_basketball(values["SITES_PLAYER_PROPS"], values["COMPETITION_PLAYER_PROPS"])
        
    middles = sorted(sb.MIDDLES.keys(), key=lambda x:trj_match(sb.MIDDLES[x])[0], reverse=True)
    surebets = sorted(sb.SUREBETS.keys(), key=lambda x:trj_match(sb.SUREBETS[x])[0], reverse=True)
    
    window["SUREBETS_PLAYER_PROPS"].update(surebets)
...
def search_props(window, values):
    try:
        perfs = sorted([x for x in sb.SUREBETS.keys() if values["SEARCH_PROPS"].lower() in x.lower()], key=lambda x:trj_with_min_odd(sb.SUREBETS[x]), reverse=True)
        window['SUREBETS_PLAYER_PROPS'].update(values=perfs)
    except:
        window['SUREBETS_PLAYER_PROPS'].update(values=[])
...
def display_surebet_info_props(window, values):
    if not values["SUREBETS_PLAYER_PROPS"]:
        return
    player_limit_market = values["SUREBETS_PLAYER_PROPS"][0]
    if player_limit_market not in sb.SUREBETS:
        return
    player, limit_market = player_limit_market.split(" / ")
    ...
    for key, value in odds.items():
        table.append([key] + list(map(lambda x : str(x).replace("1.01", "-   "), value)))
    table.sort()
    window["ODDS_PROPS"].update(table, visible=True)
    window["MATCH_PROPS"].update(sb.SUREBETS[player_limit_market]["match"])
    window["PLAYER_PROPS"].update(player)
    window["MARKET_PROPS"].update(market)
    window["OUTCOME0_PROPS"].update("Over {} @ {} : {}".format(limit, best_odds[0], bookmakers[0]))
    window["OUTCOME1_PROPS"].update("Under {} @ {} : {}".format(limit, best_odds[1], bookmakers[1]), visible=best_odds[1]>1.01)
    window["TRJ_PROPS"].update("TRJ : {}%".format(round(trj*100, 3)), visible=best_odds[1]>1.01)
    #window["PROBA_MIDDLE_PROPS"].update("")
    #window["SUM_MIDDLE_PROPS"].update("")    
...
def display_middle_info(window, values):
    ...
    print(player_down_up_market.split)
    ...
    positve_EV = (1/equivalent_odd)*(trj*2-100)
    negative_EV = (1-(1/equivalent_odd))*(100-trj)
    ret = round(positve_EV-negative_EV,2)
    
    print('postive EV={}'.format(positve_EV))
    print('negative EV={}'.format(negative_EV))
    print(ret)
    ...
    window["RET_PERF"].update("Ret. : ${}".format(ret))
    window["PROBA_MIDDLE_PERF"].update("Probabilité de middle : {} % / {} %".format(proba, round(100/equivalent_odd, 3)))
    window["SUM_MIDDLE_PERF"].update("Proba théorique - Proba bookmaker : {} %".format(proba - round(100/equivalent_odd),3), text_color="red" if proba - 100/equivalent_odd>0 else "black")
...
def sort_middle_ret(window, values):
    if not sb.MIDDLES:
        return
    def get_ret(key):
        ...
        return ret
    middles = sorted(sb.MIDDLES.keys(), key=get_ret, reverse=True)
    window["MIDDLES_PERF"].update(middles)
...
def get_best_conversion_rates_freebet(window):
    ...
    for function, field in [(best_match_freebet_GUI, "CONVERT_RATES_FREEBET"),]:# (best_matches_freebet2, "CONVERT_RATES_FREEBET_2")]:
        conversion_rates = {}
        print(sb.FREEBETS_RATES)
