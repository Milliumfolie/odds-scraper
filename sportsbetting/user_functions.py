#!/usr/bin/env python3
"""
Main betting assistant functions
"""
...
import sys 
sys.path.append(r'C:/Users/Jorden/Desktop/ebdjango')
...
from sportsbetting.auxiliary_functions import (valid_odds, format_team_names, merge_dict_odds_GUI, merge_dict_odds, afficher_mises_combine,
...
from sportsbetting.auxiliary_functions import best_match_base, best_match_base_GUI


def parse_competition(competition, sport, *sites):
    """
    Return the odds of a competition for one or several sites. 
    If no site is chosen, the parsing is done on all the bookmakers.
    """
    if sb.ABORT:
    ...
    for site in sites:
        ...
        try:
            ...
            print("{} not accessible on {} (timed out)".format(competition, site))
        except KeyboardInterrupt:
            res_parsing[site] = {}
        # except selenium.common.exceptions.TimeoutException:
        #     print("Element not found by selenium ({} on {})".format(competition, site))
        except sb.UnavailableCompetitionException:
            print("{} not available on {}".format(competition, site))
        except socket.timeout:
            print("{} not accessible on {} (timeout socket)".format(competition, site))
        # except selenium.common.exceptions.StaleElementReferenceException:
        #     print("StaleElement not found by selenium ({} on {})".format(competition, site))
        # except selenium.common.exceptions.WebDriverException:
        #     print("Connection closed ({} on {})".format(competition, site))
        except requests.exceptions.SSLError:
            print("Max retries ({} on {})".format(competition, site))
    
    print(res_parsing)
    res = format_team_names(res_parsing, sport, competition)
    out = valid_odds(merge_dict_odds(res, competition), sport)
    return out

def parse_competition_GUI(competition, sport, *sites):
    """
    ...
    res = format_team_names(res_parsing, sport, competition)
    out = valid_odds(merge_dict_odds_GUI(res), sport)
    return out
...
def parse_competitions_site(competitions, sport, site):
    ...
    try:
        ...
        print("{} not accessible".format(site))
    ...
    #print('list_odds')
    print(list_odds)
    return merge_dict_odds_GUI(list_odds)

def parse_competitions_site_GUI(competitions, sport, site):
    list_odds = []
...
def parse_competitions(competitions, sport, *sites):
    ...
    try:
        ...
        sb.ODDS[sport] = merge_dict_odds_GUI(list_odds)
    except Exception:
        print(traceback.format_exc(), file=sys.stderr)
    sb.IS_PARSING = False
    sb.ABORT = False
    sb.SEEN_SUREBET[sport] = False
    print("Last odds retrieved at", datetime.datetime.today().strftime("%H:%M"))
    

def odds_match(match, sport="football"):
    """    
    Returns the odds of a given match on all sites
    """
...
def best_stakes_match(match, site, bet, minimum_odd, sport="football"):
    """
    For a match, a bookmaker, a amount to bet on this bookmaker and
    minimum odds given, returns the best combination of bets to place
    """
...
def best_match_under_conditions(site, minimum_odd, bet, competition, sport="football", date_max=None,
    ...
    result_function = lambda best_overall_odds, best_rank: (mises2(best_overall_odds, bet,
                                                                   best_rank, False) if not one_site
                                                                       ...
                                                                       False))
    result, summary = best_match_base(odds_function, profit_function, criteria, display_function,
                    result_function, site, competition, sport, date_max, time_max, date_min,
                    time_min, one_site=one_site)
    
    return result, summary


def best_match_under_conditions_GUI(site, minimum_odd, bet, sport="football", date_max=None,
                                time_max=None, date_min=None, time_min=None, one_site=False):
    """
    Retourne le meilleur match sur lequel miser lorsqu'on doit miser une somme
    donnée à une cote donnée. Cette somme peut-être sur seulement une issue
    ...
    result_function = lambda best_overall_odds, best_rank: (mises2(best_overall_odds, bet,
                                                                   best_rank, False) if not one_site
                                                                       ...
                                                                       False))
    best_match_base_GUI(odds_function, profit_function, criteria, display_function,
...
def best_match_freebet(site, freebet, competition=None, sport="football", live=False, date_max=None, time_max=None,
...
def best_match_freebet_GUI(site, freebet, sport="football", live=False, date_max=None, time_max=None,
                       date_min=None, time_min=None):
    """
    Retourne le match qui génère le meilleur gain pour un unique freebet placé,
    couvert avec de l'argent réel.
...
def best_matches_freebet_one_site(site, freebet, sport="football", nb_matches=2,
    ...
    print("here before best matches")
    print(all_odds)
...
def gain_middle(cotes, mise=1):
    ...
    :rtype: int
    """
    return (cotes[0] / sum(cotes))*cotes[1]*100*2


def ret_match(match_odds):
    odds = []
    bookmakers = []
    for bookmaker in match_odds["odds"]:
        if bookmaker == "unibet_boost":
    ...
    return gain_middle(odds), bookmakers, odds
...
def best_matches_freebet2(site, freebet, sport, nb_matches=2):
    ...
    for combi in combis:
        ...
        for choice in choices:
            ...
            best_odd = odd
    tables, summary = best_combine_reduit(best_matches, best_choice, site, freebet, sport, best_odd-1, freebet=True)
    print(tables)
...
if __name__ == "__main__":
    test1, test2 = best_match_freebet('betcity', 10, competition=None, sport="football", live=False, date_max=None, time_max=None, date_min=None, time_min=None)
