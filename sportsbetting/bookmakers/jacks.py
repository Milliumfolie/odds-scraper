...
def parse_jacks_api(url):
    """
    Get jacks odds from league id and sport.
    Args:
        url (str): The URL to the jacks API.
    Returns:
        dict: A dictionary containing the parsed odds data.
    Raises:
        requests.exceptions.RequestException: If the request to the API fails.
    """
    sport = get_sport_by_url(url, 'jacks')
...
def parse_jacks(url):
    """
    Get jacks odds from url
    """
    if url:
        return parse_jacks_api(url)
    print("Wrong jacks url")
    return {}


def get_sub_markets_players_basketball_jacks(id_match):
    """
    ...
    for market_class_list in markets_class_list:
        ...
        #                                             .split(" - ")[0]
        #                                             .replace(" ", "")))
        # id_team = is_in_db_site(team_name, "basketball", "jacks")
        # if id_team:
        ...
        for selection in selections:
            ...
            if not is_3_pts:
                ...
                ref_player = player
                if is_player_added_in_db(player, "jacks"):
                    ref_player = is_player_added_in_db(player, "jacks")
                elif is_player_in_db(player):
                    add_player_to_db(player, "jacks")
                else:
                    if sb.DB_MANAGEMENT:
                        print(player, "jacks")
                        add_close_player_to_db(player,"jacks")
                    else:
                        continue
            ...
            if key_player not in sub_markets[key_market]:
                sub_markets[key_market][key_player] = {"odds":{"jacks":[]}}
            if plus:
                sub_markets[key_market][key_player]["odds"]["jacks"].insert(0, odd)
            else:
                sub_markets[key_market][key_player]["odds"]["jacks"].append(odd)
            if key_market == "Points":
                sub_markets[key_market][key_player]["odds"]["jacks"].append(1.01)
    for sub_market in sub_markets:
        sub_markets[sub_market] = dict(sub_markets[sub_market])
    return sub_markets


def get_sub_markets_football_jacks(id_match):
    """
    ...
    url = f'https://eu1.offering-api.kambicdn.com/offering/v2018/jvh/betoffer/event/{id_match}.json?lang=en_GB&market=NL'
    response = requests.get(url)
    data = response.json()
    ...
    id_team_A = is_in_db_site(team_A, "football", "jacks")
    id_team_B = is_in_db_site(team_B, "football", "jacks")
    ...
    hedgeable_markets = defaultdict(lambda: defaultdict(lambda: {"odds": {"jacks": []}, "limit": ""}))
    for bet_offer in bet_offers:
        ...
        if (len(outcomes) == 2 and all("odds" in outcome for outcome in outcomes)):
            ...
            for outcome in bet_offer["outcomes"]:
                ...
                if outcome["type"] == "OT_OVER":
                    hedgeable_markets[general_name][odds_key]["odds"]["jacks"].insert(0, odds_value)
                else:
                    hedgeable_markets[general_name][odds_key]["odds"]["jacks"].append(odds_value)
...
def get_player_props_football_jacks(id_match):
    """
    ...
    sub_markets = {v: defaultdict(lambda: {"odds": {"jacks": []}, "limit": ""}) for v in markets_to_keep.values()}
    for market_class_list in markets_class_list:
        ...
        if market_name in markets_to_keep:
            ...
            for selection in selections:
                ...
                if plus:
                    sub_market[player]["odds"]["jacks"].insert(0, odd)
                else:
                    sub_market[player]["odds"]["jacks"].append(odd)
...
if __name__ == "__main__":
    test = parse_jacks("https://eu-offering.kambicdn.org/offering/v2018/jvh/listView/football/netherlands/eredivisie/all/matches.json?lang=nl_NL&market=NL")