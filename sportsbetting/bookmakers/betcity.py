...
def parse_betcity_api(url):
    """
    Get Betcity odds from league id and sport.
    Args:
        url (str): The URL to the Betcity API.
    Returns:
    ...
    sport = get_sport_by_url(url, 'betcity')
...
def parse_betcity(url):
    """
    Get Betcity odds from url
    """
    if url:
        return parse_betcity_api(url)
...
def get_ref_player(market_name, team_A, team_B):
    if market_name == 'Total Goals':
        ...
        id_team = is_in_db_site(team_name, 'football', 'betcity')
        if id_team:
        ...
        ref_player = player
        if is_player_added_in_db(player, 'betcity'):
            ref_player = is_player_added_in_db(player, 'betcity')
        elif is_player_in_db(player):
            add_player_to_db(player, 'betcity')
        else:
            if sb.DB_MANAGEMENT:
                print(player, 'betcity')
                add_close_player_to_db(player, 'betcity')
            else:
                return 'Match'
        return ref_player
    else:
...
def get_sub_markets_players_basketball_betcity(id_match):
    """
    Get submarkets odds from basketball match
    """
    if not id_match:
        return {}
    url = 'https://eu-offering.kambicdn.org/offering/v2018/betcitynl/betoffer/event/' + str(id_match) + '.json?lang=nl_NL&market=NL'
    ...
    for market_class_list in markets_class_list:
        ...
        for selection in selections:
            ...
            if key_player not in sub_markets[key_market]:
                sub_markets[key_market][key_player] = {"odds":{"betcity":[]}}
            if plus:
                sub_markets[key_market][key_player]["odds"]["betcity"].insert(0, odd)
            else:
                sub_markets[key_market][key_player]["odds"]["betcity"].append(odd)
            if key_market == "Points":
                sub_markets[key_market][key_player]["odds"]["betcity"].append(1.01)
    for sub_market in sub_markets:
        sub_markets[sub_market] = dict(sub_markets[sub_market])
    return sub_markets


def get_sub_markets_football_betcity(id_match):
    """
    ...
    url = f'https://eu1.offering-api.kambicdn.com/offering/v2018/betcitynl/betoffer/event/{id_match}.json?lang=en_GB&market=NL'
    response = requests.get(url)
    data = response.json()
    ...
    id_team_A = is_in_db_site(team_A, "football", "betcity")
    id_team_B = is_in_db_site(team_B, "football", "betcity")
    ...
    hedgeable_markets = defaultdict(lambda: defaultdict(lambda: {"odds": {"betcity": []}, "limit": ""}))
    for bet_offer in bet_offers:
        ...
        if (len(outcomes) == 2 and all("odds" in outcome for outcome in outcomes)):
            ...
            for outcome in bet_offer["outcomes"]:
                ...
                if outcome["type"] == "OT_OVER":
                    hedgeable_markets[general_name][odds_key]["odds"]["betcity"].insert(0, odds_value)
                else:
                    hedgeable_markets[general_name][odds_key]["odds"]["betcity"].append(odds_value)
...
def get_player_props_football_betcity(id_match):
    """
    ...
    url = f'https://eu1.offering-api.kambicdn.com/offering/v2018/betcitynl/betoffer/event/{id_match}.json?lang=nl_NL&market=NL'
    ...
    sub_markets = {v: defaultdict(lambda: {"odds": {"betcity": [1.01, 1.01]}, "limit": ""}) for v in markets_to_keep.values()}
    for market_class_list in markets_class_list:
            ...
            for outcome in outcomes:
                if outcome.get('status') == 'OPEN':
                    ...
                    if is_player_in_db(participant):
                        participant = is_player_added_in_db(participant, "betcity")
                    else:
                        add_new_player_to_db(participant)
                        add_player_to_db(participant, "betcity")
                        participant = is_player_added_in_db(participant, "betcity")
                    ...
                    if participant:
                        ...
                        if key not in sub_markets[market_naam]:
                            sub_markets[market_naam][key] = {'odds': {'betcity': [1.01, 1.01]}, 'limit': line}
                        if outcome_label == 'Over':
                            sub_markets[market_naam][key]['odds']['betcity'][0] = odds
                        else:
                            sub_markets[market_naam][key]['odds']['betcity'][1] = odds
