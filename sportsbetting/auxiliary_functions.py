...
def add_matches_to_db(odds, sport, site, id_competition):
    ...
    :param sport: Sport
    :param site: Name of the bookmaker
...
def merge_dict_odds_GUI(dict_odds, needs_date=True):
    """
...
def afficher_mises_combine(matches, sites, list_mises, cotes, sport="football",
    ...
    print("\nDistribution of stakes (the totals displayed take into account the "
          "possible freebets):")
    print(rang_freebet)
    ...
    # Instead of printing, create dictionaries and return them
    table_data = []
    for team, bookmaker, odds, stakes, total in zip(table_teams, table_bookmakers, table_odds, table_stakes, table_totals):
        for sub_team, sub_bookmaker, sub_odds, sub_stakes in zip(team.split(' / '), bookmaker.split('\n'), odds.split('\n'), stakes.split('\n')):
            row = {
                'Issue': sub_team,
                'Bookmaker': sub_bookmaker,
                'Cote': sub_odds,
                'Mise': sub_stakes,
                'Total': total
            }
    ...
    summary_data = {
        "TRJ": str(round(trj*100, 2))+"%",
        "Plus-value": round(profit, 3),
        "Compétition": sb.ODDS[sport].get(match, {}).get("competition", "Indéterminée")
    }

    return table_data, summary_data


def afficher_mises_combine_GUI(matches, sites, list_mises, cotes, sport="football",
                           rang_freebet=None, uniquement_freebet=False,
                           cotes_boostees=None, rang_2e_freebet=-1, combinaisons=None, profit=0):
    """
...
def best_match_base(odds_function, profit_function, criteria, display_function,
                    result_function, site, competition=None, sport="football", date_max=None,
    ...
    # json_file_path = r'C:/Users/Jorden/Desktop/ebdjango/ebdjango/data.json'
    ...
    try:
        if combine:
            ...
            if competition:
                print(competition)
                all_odds = filter_dict_competition(sb.ODDS["football"], competition)
            else:
    ...
    if total:
        if combine_opt and combine:
            ...
            #sort by profit and extract first 15
            total = dict(sorted(total.items(), key=lambda item: item[1]['profit'], reverse=True))
            total = dict(list(total.items())[:15])
            
            for key, item in total.items():
                #second_rank = display_function(best_overall_odds, best_rank)
                second_rank = display_function(item['odds_to_check'], item['rank'])
                
                print(key)
                table_data[key], summary_data[key] = afficher_mises_combine([key], item['sites'],
                                                        [result_function(item['odds_to_check'], item['rank'])],
                                                        all_odds[key]["odds"], sport, item['rank'] if freebet else None,
                                                        one_site and freebet, best_overall_odds, second_rank, profit=item['profit'])
...
def best_match_base_GUI(odds_function, profit_function, criteria, display_function,
        ...
        if combine_opt and combine:
            ...
            if recalcul:
                ...
                find_almost_won_matches(best_match, result_function(best_overall_odds, best_rank),
                                        sport, True)
            second_rank = display_function(best_overall_odds, best_rank)
            afficher_mises_combine_GUI(best_match.split(" / "), [sites],
                                   [result_function(best_overall_odds, best_rank)],
...
def filter_dict_competition(odds, competition):
    all_odds = copy.deepcopy(odds)    
    all_odds = {key: value for key, value in all_odds.items() if value["competition"] == competition}
    return all_odds
